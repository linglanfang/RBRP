...
use vars qw ($opt_h $opt_V $opt_D $opt_i $opt_o $opt_t $opt_T $opt_R $opt_s $opt_e );
&getopts('hVDi:o:t:T:R:s:e:');
...
sub main 
{
	...
	fgRTstopThreshold => $parameters{fgRTstopThreshold},
...
sub init 
{
    ...
    if ( defined $opt_R )  { $parameters{fgRTstopThreshold} = $opt_R; }
    else { $parameters{fgRTstopThreshold} = 0;  }
...
sub readSignals 
{
    ...
    my $fgRTstopThreshold = $parameters{fgRTstopThreshold};
        ...
        if ( $coverage > $coverageThreshold ) {
            ...
            for ( my $idxPos = 0; $idxPos < $len; $idxPos++ ) {
                ...
                if ( ( $idxPos > $headToSkip ) and ( $idxPos < ( $len - $tailToSkip ) ) )  {
                    if ( ( $bgBDs[$idxPos] ne "NULL" ) and ( $bgBDs[$idxPos] >= $winThreshold ) and ( $fgrtstops[$idxPos] >= $fgRTstopThreshold) )  { $selected = 1; }
                }
