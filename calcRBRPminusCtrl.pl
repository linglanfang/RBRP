#! /usr/bin/perl
# Note: The script below is modified based on previously published script from https://github.com/qczhang/icSHAPE (Nature, 2015, 486–490)
# Calculate RBRP enrichment file using (fg_RTstop/fg_BD) - (bg_RTstop/bg_BD), if fg_BD > 0 and bg_BD > 0
#
use strict;
use warnings;
use File::Basename;
...
sub calcEnrichment {
    ...
    foreach my $trans ( keys %{$ref_fg_RTstop} ) {
        ...
        if ( $ref_parameters->{logspace} or ( $enrichMethod eq "subtraction" ) )  {
            ...
            #next if ( ( not defined $ref_bg_baseDensity->{$trans} ) and ( not defined $ref_fg_baseDensity->{$trans}) );
            for ( my $idx = 0; $idx < $skipHead; $idx++ )  {
                $siganlEnrichment[$idx] = "NULL";
            }
            for ( my $idx = $skipHead; $idx < ( $ref_fg_len->{$trans} - $skipTail )-1; $idx++ )  {
                if ( ( $ref_bg_baseDensity->{$trans}[$idx+1] > 0 ) and ( $ref_fg_baseDensity->{$trans}[$idx+1] > 0 ) ) {
                    $siganlEnrichment[$idx] = sprintf ( "%.3f", ( ( ( $ref_fg_RTstop->{$trans}[$idx] ) / $ref_fg_baseDensity->{$trans}[$idx+1] ) - ($divFactor * ( $ref_bg_RTstop->{$trans}[$idx] ) / $ref_bg_baseDensity->{$trans}[$idx+1]) ) );
                }
                else {
                    $siganlEnrichment[$idx] = "NULL";
                }
...
sub outputEnrichment  {
    ...
    foreach my $trans ( keys %{$ref_transcript_enrichment} )  {
        ...
        for ( my $idx = 0; $idx < $ref_transcript_len->{$trans}; $idx++ ) {
            ...
            print OUT ",", sprintf ( "%.3f", ( $ref_fg_RTstop->{$trans}[$idx] ) );
            print OUT ",", sprintf ( "%.3f", ( $ref_bg_RTstop->{$trans}[$idx] )  );
            print OUT ",", sprintf ( "%.3f", ( $ref_bg_baseDensity->{$trans}[$idx] )  );
        }
        print OUT "\n";
    }
    close OUT;
